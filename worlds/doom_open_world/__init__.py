...
from .Options import DOOMOpenWorldOptions

logger = logging.getLogger("DOOM Open World")

DOOM_TYPE_LEVEL_COMPLETE = -2
DOOM_TYPE_COMPUTER_AREA_MAP = 2026


class DOOMOpenWorldLocation(Location):
    game: str = "DOOM Open World"


class DOOMOpenWorldItem(Item):
    game: str = "DOOM Open World"


class DOOMOpenWorldWeb(WebWorld):
    tutorials = [Tutorial(
        "Multiworld Setup Guide",
        "A guide to setting up the DOOM Open World randomizer connected to an Archipelago Multiworld",
        "English",
        "setup_en.md",
        "setup/en",
        ["Daivuk"]
...
class DOOMOpenWorldWorld(World):
    """
    Developed by id Software, and originally released in OpenWorld, DOOM pioneered and popularized the first-person shooter,
    setting a standard for all FPS games.
    """
    options_dataclass = DOOMOpenWorldOptions
    options: DOOMOpenWorldOptions
    game = "DOOM Open World"
    web = DOOMOpenWorldWeb()
    data_version = 3
    ...
    def create_item(self, name: str) -> DOOMOpenWorldItem:
        item_id: int = self.item_name_to_id[name]
        return DOOMOpenWorldItem(name, Items.item_table[item_id]["classification"], item_id, self.player)

    def create_items(self):
        itempool: List[DOOMOpenWorldItem] = []
    ...
    def create_ratioed_items(self, item_name: str, itempool: List[DOOMOpenWorldItem]):
    ...
    def fill_slot_data(self) -> Dict[str, Any]:
        slot_data = self.options.as_dict("difficulty", "random_monsters", "random_pickups", "random_music", "flip_levels", "allow_death_logic", "pro", "start_with_computer_area_maps", "death_link", "episode1", "episode2", "episode3", "episode4")
